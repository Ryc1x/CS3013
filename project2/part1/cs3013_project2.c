...
asmlinkage long new_sys_read(int fd, void* buf, size_t count) {
  // get the id
  int id = current_uid().val;
  int size = ref_sys_read(fd, buf, count);
  // check VIRUS if user called open
  if (id >= 1000){
    if (size == 0){
      printk(KERN_INFO "User %d reads the file descriptor: %d.(empty)\n", id, fd);
      return size;
    }
    if (strstr((char*) buf, "VIRUS")) {
      printk(KERN_INFO "User %d reads from file descriptor: %d, but that read contained malicious code!\n", id, fd);
      return size;
    }
  }
  return size;
